 ...
 * 
 * <br>
 * <b>功能：</b>ZcInController<br>
 *   <br>
 */ 
@Controller
@RequestMapping("/zcIn")
public class ZcInCtl extends BaseCtl{
	
	private final static Logger log= Logger.getLogger(ZcInCtl.class);
	...
	public String getZcInList (HttpServletRequest req,ZcInVo vo) throws Exception{
	    	String json = null;
		try {
			List<ZcIn> list = zcInService.selectZcInByPage(vo);
			int totalCount = zcInService.selectZcInCount(vo);
	...
	public String save(@RequestBody String json) {
		...
		try {
			log.debug("---json---:"+json);	
			ZcIn zcIn = JSON.parseObject(json,ZcIn.class);
			
			int i = zcInService.insertZcIn(zcIn);
	...
	public String delete(@RequestBody String json) {  
		...
		try {
			log.debug("===json="+json);	
			ZcInVo zcIn = JSON.parseObject(json,ZcInVo.class);
			log.debug("===json="+zcIn.getIdList());
			int i = 0;
			//批量删除，单个删除特例
			if (zcIn.getIdList().size() > 0)
			{
				i = zcInService.deleteZcInByBatch(zcIn.getIdList());
			}
	...
	public String edit(@RequestBody String json) {
		...
		try {
			log.debug("===json="+json);	
			ZcIn zcIn = JSON.parseObject(json,ZcIn.class);
			int i = zcInService.updateZcInByPrimaryKeySelective(zcIn);
