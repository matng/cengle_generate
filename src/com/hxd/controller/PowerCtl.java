...
public class PowerCtl extends BaseCtl{
	...
	private final static Logger log= Logger.getLogger(PowerCtl.class);
	...
	public String getPowerList (HttpServletRequest req,PowerVo vo) throws Exception{
	    	String json = null;
		try {
			List<Power> list = powerService.selectPowerByPage(vo);
			int totalCount = powerService.selectPowerCount(vo);
	...
	public String delete(@RequestBody String json) {  
		...
		try {
			log.debug("===json="+json);	
			PowerVo power = JSON.parseObject(json,PowerVo.class);
			log.debug("===json="+power.getIdList());
			int i = 0;
			//批量删除，单个删除特例
			if (power.getIdList().size() > 0)
			{
				i = powerService.deletePowerByBatch(power.getIdList());
			}
	...
	public String edit(@RequestBody String json) {
		...
		try {
			log.debug("===json="+json);	
			Power power = JSON.parseObject(json,Power.class);
			int i = powerService.updatePowerByPrimaryKeySelective(power);
