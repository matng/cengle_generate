...
import com.hxd.vo.CompanyDeptVo;
...
@Transactional
public class CompanyDeptServiceTest {
	...
	public void testAdd(){
		try {
			...
			companyDeptService.insertCompanyDept(companyDept);
			id = companyDept.getId();
	...
	public void testList(){
		try {
			...
			companyDeptVo.setiDisplayLength(companyDeptService.selectCompanyDeptCount(companyDeptVo));
			List<CompanyDept> companyDeptList = companyDeptService.selectCompanyDeptByPage(companyDeptVo);
			System.out.println("返回总条数："+companyDeptList.size());
			for(CompanyDept e : companyDeptList){
	...
	public void testDel(){
		try {
			companyDeptService.deleteCompanyDeptByPrimaryKey((Integer)id);
